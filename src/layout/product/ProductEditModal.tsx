...
export default function ProductEditModal({ show, onClose, product, onSubmit }: ProductEditModalProps) {
  ...
  const handleChange = (field: keyof Product, value: string | number) => {
    setEditable((prev) =>
      prev ? { ...prev, [field]: field === "stock" ? parseInt(value as string || "0") : value } : prev
    );
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      let imageUrl = editable.image;
      if (imageFile) {
        imageUrl = await uploadProductImage(imageFile);
      }

      const updatedProduct = { ...editable, image: imageUrl };
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          {["code", "name", "description", "size"].map((f) => (
            <div key={f}>
              ...
              <TextInput id={f} value={(editable as any)[f] ?? ""} onChange={(e) => handleChange(f as keyof Product, e.target.value)} />
            </div>
          ))}

          <div>
            <Label htmlFor="stock">Stocks</Label>
            <TextInput id="stock" type="number" value={editable.stock ?? 0} onChange={(e) => handleChange("stock", e.target.value)} />
          </div>

          <div>
            <Label htmlFor="status">Status</Label>
            <Select id="status" value={editable.status} onChange={(e) => handleChange("status", e.target.value)}>
          ...
          <div>
            ...
            <TextInput id="expiryDate" type="date" value={editable.expiryDate ?? ""} onChange={(e) => handleChange("expiryDate", e.target.value)} />
          </div>

          <div>
            <Label htmlFor="inDate">In Date</Label>
            <TextInput id="inDate" type="date" value={editable.inDate ?? ""} onChange={(e) => handleChange("inDate", e.target.value)} />
          </div>
        </div>
      </ModalBody>
